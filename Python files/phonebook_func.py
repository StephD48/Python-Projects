...
def addToList(self):
    ...
    var_fullname = ("{} {}".format(var_fname,var,var_lname)) # combine our normalized names into a full name
...
def onDelete(self):
    var_select = select.lstList1.get(self.lstList1.curselection()) # Listbox's selected value
    conn = sqlite3.connect('db_phonebook.db')
    with conn:
        cur = conn.cursor()
        ...
        count = cur.fetchone()[0]
        if count > 1:
            confirm = messagebox.askokcancel("Delete Conformation", "All information associated with, ({}) \nwill be permenantly deleted from the database. \n\nProceed with the deletion request?".format(var_select))
            if confirm:
                conn = sqlite3.connect('db_phonebook.db')
                with conn:
                    cursor = conn.cursor()
